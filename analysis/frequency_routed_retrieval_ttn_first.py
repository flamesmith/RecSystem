"""Frequency-routed retrieval, TTN-first variant: split each query's
candidate pool by the CANDIDATE's own training-target frequency, but instead
of a proportional slot split (frequency_routed_retrieval.py, which lost to
plain TTN at every threshold -- see that file's log), give TTN first claim
on all K slots and only backfill with CONTENT when TTN's own pool is too
small to fill them.
...
Mechanism, per test pair (query, target_node):
  1. All candidates in the target's category are split into WARM (target_freq
     ...
     per-query decision.
  2. TTN-first: take TTN's top min(K, n_warm) from the warm pool. Only if
     that's fewer than K (the warm pool itself has fewer than K candidates --
     not "TTN ranked them low", but there simply aren't K warm items in this
     category) does CONTENT backfill the remaining slots from the cold pool.
     When n_warm >= K (the common case), this is IDENTICAL to plain TTN --
     differences from plain TTN only show up in categories too small or too
     cold for TTN to fill the list on its own.
...
THRESHOLD is swept over the same values as the proportional variant, next to
plain TTN+IMAGE, plain CONTENT (desc+title+features+image, 0.25 each -- this
session's best of each), and POP for reference.

Usage: python ttn/experiments/frequency_routed_retrieval_ttn_first.py
"""
import json
import time
import warnings
    ...
    for r, tp in enumerate(t_pos):
        ...
        for thr in THRESHOLDS:
            ...
            for k, hits in ((10, hit10), (100, hit100)):
                if n_warm + n_cold == 0:
                    continue
                # TTN-first: claim up to k slots from the warm pool; CONTENT
                # only backfills what TTN's own pool is too small to fill.
                k_warm = min(k, n_warm)
...
table(hit10, f"Recall@10  (test sample {len(sample):,} pairs, seed {SEED}) "
             f"-- TTN-first routing; TTN=TTN+IMAGE, CONTENT=desc+title+features+image")
