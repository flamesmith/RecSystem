"""Add SigLIP2 image embeddings to (1) the TTN model and (2) the CONTENT
signal, per this session's brief: "include the embeddings to the TTN model
and the desc + title + features (give each one .25 weight)".
...
final_capacity_comparison.py, to stay consistent with how the rest of this
session's analysis (reproduce_section19.py, content_desc_title_features.py)
was done: full-data training, frequency buckets from the full training set.

The image branch is patched into the model-code cell exactly as
final_capacity_comparison.py does it (same anchors, same architecture), only
without shrinking EPOCHS or swapping in the 20k-pair sample.

IMPORTANT: saves the new checkpoint to a SEPARATE file
(data/tower/ttn_complementary_image.pt), not over the committed
ttn_complementary.pt -- that file is relied on elsewhere (the notebook, every
"TTN" column scored so far in this session) and this is an additive
...
DESC and POP are unchanged from the rest of this session's tables.

Usage: python ttn/experiments/image_augmented_models.py
"""
import json
import time
import warnings
...
print(f"TTN (original checkpoint) scored on test sample")

# =============================================================================
# report
# =============================================================================
COLS = ["TTN", "TTN+IMAGE", "DESC", "POP", "CONTENT"]
