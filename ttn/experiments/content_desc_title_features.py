"""§19 rerun with CONTENT redefined as description + title + features
(instead of §19's description + colour + material), per this session's brief.

Same methodology as ttn/experiments/reproduce_section19.py in every other
respect, so the two tables are directly comparable:
  - TTN: the committed checkpoint (data/tower/ttn_complementary.pt).
  - DESC: description-only SBERT cosine, top-k membership scoring.
  - POP: membership in the target's category's top-10 / top-100 most frequent
    training targets.
  - CONTENT: mean cosine over whichever of {description, title, features} is
    present on both query and candidate -- description and title are the
    model's own frozen SBERT embeddings (items.npz), features is the
    extracted-attribute text, SBERT-encoded the same way as §13's
    final_capacity_comparison.py DESC+TITLE+FEAT combo.
  - Candidates restricted to the pair's target category, self excluded.
  - freq = the target's count as a target across the full training pairs
    table (2,223,995 pairs).
  - Evaluation: 20,000 pairs sampled from pairs_test.parquet, seed 0 --
    same sample as reproduce_section19.py.

Usage: python ttn/experiments/content_desc_title_features.py
"""
import json
import time
import warnings
...
print(f"description present: {has_desc.mean():.1%}   title present: {has_title.mean():.1%}")


# --- features text, read back from the model's own vocabulary --------------
def text_for(field):
...
print(f"features present: {has_features.mean():.1%}")
print(f"encoded features in {time.time() - t0:.0f}s total")
...
# --- 20,000-pair test sample, seed 0 (same draw as reproduce_section19.py) -
...
table(hit10, f"Recall@10  (n={len(sample):,} sampled test pairs, seed {SEED}) -- CONTENT = desc+title+features")
table(hit100, f"Recall@100  (n={len(sample):,} sampled test pairs, seed {SEED}) -- CONTENT = desc+title+features")
