"""Frequency-routed retrieval, reserved-slots variant: fixes the TTN-first
variant's failure mode (frequency_routed_retrieval_ttn_first.py: CONTENT only
got a turn when a category's ENTIRE warm pool ran short of K, so a query
whose own correct answer was cold had almost no channel to surface it, and
cold-bucket recall got WORSE than plain TTN). Here CONTENT always gets a
fixed slot reservation, on every query, regardless of how many warm
candidates the category has.
...
Mechanism, per test pair (query, target_node):
  1. All candidates in the target's category are split into WARM (target_freq
     ...
     per-query decision.
  2. RESERVE_FRAC of the K slots (rounded) are reserved for CONTENT's top
     picks from the cold pool, ALWAYS -- not only when TTN's pool falls
     short. The rest go to TTN's top picks from the warm pool. If either
     pool can't fill its share, the shortfall rolls over to the other pool
     (so a category that's 100% warm still gets a full K-length TTN list,
     and a category that's 100% cold still gets a full K-length CONTENT
     list) -- the reservation is a guarantee for CONTENT, not a cap.
...
RESERVE_FRAC is fixed at 0.2 (2 of 10 slots, 20 of 100) -- a reasonable
starting guess, not swept here. THRESHOLD is swept over the same values as
the other two variants, next to plain TTN+IMAGE, plain CONTENT
(desc+title+features+image, 0.25 each), and POP for reference.

Usage: python ttn/experiments/frequency_routed_retrieval_reserved.py
"""
import json
import time
import warnings
    ...
    for r, tp in enumerate(t_pos):
        ...
        for thr in THRESHOLDS:
            ...
            for k, hits in ((10, hit10), (100, hit100)):
                if n_warm + n_cold == 0:
                    continue
                # CONTENT gets a fixed reservation on every query; TTN gets
                # the rest. Either shortfall rolls over to the other pool so
                # the merged list still reaches k when the category allows.
                k_cold = min(int(round(k * RESERVE_FRAC)), n_cold)
                k_warm = min(k - k_cold, n_warm)
                if k_warm < k - k_cold:
                    k_cold = min(k - k_warm, n_cold)
...
table(hit10, f"Recall@10  (test sample {len(sample):,} pairs, seed {SEED}) "
             f"-- {RESERVE_FRAC:.0%} reserved for CONTENT; TTN=TTN+IMAGE, CONTENT=desc+title+features+image")
