"""Drills into the two categories the user flagged as most interesting from
query_side_breakdown.py's cat_2 table -- Furniture and Bedding -- filtered
from the SAME 20k-pair test sample (seed 0) used throughout this session, and
re-bucketed by the TARGET's training frequency (never/1-5/6-25/26-100/>100,
computed from pairs_train's target_idx column -- the same definition used in
every bucketed table before query_side_breakdown.py), so it's directly
comparable to the original full-sample target-frequency table, just narrowed
to these two categories.

Two tables, one per category (Furniture, Bedding), each showing the five
models (TTN, TTN+IMAGE, DESC, POP, CONTENT) broken out by target frequency.
Bucket sizes are small at this filter (each category is ~1,250-2,250 of the
20,000-pair sample, so some buckets carry only a few dozen pairs) -- reported
as-is, with pair counts alongside every row so low-n cells are visible rather
than hidden.
...
Usage: python ttn/experiments/furniture_bedding_breakdown.py
"""
import json
import time
import warnings
...
# --- Both frequency definitions: TARGET-side (how often this item was the
# correct answer in training) and QUERY-side (how often it was the item
# being looked at) -- reported as two separate breakdowns below.
...
print("query cat_2 counts:")
print(sample["qcat2"].value_counts())
print("\nFurniture pairs:", int((sample['qcat2'] == 'Furniture').sum()),
      "| Bedding pairs:", int((sample['qcat2'] == 'Bedding').sum()))
...
# =============================================================================
# report -- restricted to query cat_2 in {Furniture, Bedding}, bucketed by
# the TARGET's training frequency within each category
# =============================================================================
def table_by(hits, base_mask, key, order_, title):
    print(f"\n{title}")
    header = f"{'group':<14}{'pairs':>8}" + "".join(f"{m:>12}" for m in MODELS)
    print(header)
    for b in order_:
        m_ = base_mask & ((sample[key] == b) if b != "all" else True)
        n = int(m_.sum())
        row = f"{str(b):<14}{n:>8}"
        for mod in MODELS:
            v = hits[mod][sample.index[m_]].mean() if n > 0 else float("nan")
            row += f"{v:>12.4f}"
        print(row)


for cat in ["Furniture", "Bedding"]:
    mask = sample["qcat2"] == cat
    n_cat = int(mask.sum())
    table_by(hit10, mask, "bucket", BUCKET_ORDER,
             f"Recall@10, query cat_2={cat}  (n={n_cat:,}, seed {SEED}), by TARGET frequency")
    table_by(hit100, mask, "bucket", BUCKET_ORDER,
             f"Recall@100, query cat_2={cat}  (n={n_cat:,}, seed {SEED}), by TARGET frequency")
    table_by(hit10, mask, "qbucket", BUCKET_ORDER,
             f"Recall@10, query cat_2={cat}  (n={n_cat:,}, seed {SEED}), by QUERY frequency")
    table_by(hit100, mask, "qbucket", BUCKET_ORDER,
             f"Recall@100, query cat_2={cat}  (n={n_cat:,}, seed {SEED}), by QUERY frequency")
