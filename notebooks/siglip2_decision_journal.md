...
## 2026-09-08 — Taxonomy-constrained complementary furniture retrieval

### Decision: begin with constrained similarity as a complement baseline

I created `siglip2_furniture_complementary_retrieval.ipynb` to test whether products can be treated as possible complements when they belong to the same room-level furniture group but a different exact product type. For a sofa in `Living Room Furniture`, candidates may include ottomans, coffee tables, chairs, poufs, or media furniture, while other `Sofas & Couches` are excluded.

What this optimizes: a simple, interpretable first experiment that uses the taxonomy and SigLIP2 representations already available. It requires no behavioral data or new training.

### Decision: use the normalized room field instead of moving one literal path level upward

The hierarchy has variable depth. A sofa path may end directly below `Living Room Furniture`, while a coffee table path includes the additional parent `Tables`. The notebook therefore uses `furniture_subcategory` as the room constraint and `leaf_category` as the excluded self category.

What this optimizes: consistent behavior across uneven taxonomy paths. Both sofas and coffee tables correctly search within the living-room candidate pool.

### Decision: evaluate four embedding comparisons

The primary comparisons are image-to-image and text-to-text similarity. The notebook also supports image-to-text and text-to-image inspection because SigLIP2 places both modalities in a shared embedding space. All rankings use cosine similarity through the dot product of unit-normalized vectors.

What this optimizes: visibility into whether visual cues, written attributes, or cross-modal agreement produce more sensible complements.

### Decision: reuse the held-out multi-chunk catalog embeddings

The notebook reads the 500 unseen catalog products and the `multichunk64` image and description embeddings generated by the text-strategy benchmark. It does not retrain or repeatedly encode the same catalog.

What this optimizes: fast iteration and an honest held-out exploration. The notebook completed all 19 cells in about 13 seconds during validation.

### Decision: make category diversification optional

The raw nearest-neighbor list can be dominated by one allowed leaf category, such as chairs. The default `max_per_leaf=2` cap exposes a wider range of possible complement types, while setting it to `None` restores the untouched similarity ranking.

What this optimizes: useful visual inspection without hiding the underlying retrieval behavior. The diversification rule is explicit and reversible.

### Important limitation: similarity is not the same as complementarity
